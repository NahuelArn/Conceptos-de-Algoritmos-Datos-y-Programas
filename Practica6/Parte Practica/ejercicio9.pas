...
la lista podría no estar ordenada. [maso]
...
procedure eliminar(L: lista; num: integer; var ok: Boolean);
var
  act,ant: lista;
begin
  ok:= false;
  ...
  While(act <> nil) and (act^.num <> num) do
    ...
    if(act <> nil )then //encontre el nodo
      begin
        ok:= true;
        if(act = L)then //era el primero
          begin
            L:= act^.sig;
          end
        else 
...
begin
  ...
  while (valor <> 0) do 
    ...
    // a mi me cambia en algo que la lista no este ordenada? en la carga podria tener podria tambien retornar el puntero al ultimo nodo y hacer una especie de busqueda dicotomica con listas 
    q me retorne la posicion si se encuentra y despues mandarlo a eliminar nodo
    }
    Writeln('Ingrese un valor a buscar en los nodos de enteros y eliminarlo: ');
    readln(valor); //sobrescribo valor
    eliminar(pri,valor,ok);
    //C
    {c. Sublista: recibe como parámetros la lista L y dos valores enteros A y B, y retorna una nueva lista con todos
    los elementos de la lista L mayores que A y menores que B.}
