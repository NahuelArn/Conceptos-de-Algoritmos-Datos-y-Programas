...
program validadorDeCriteriorDeOrden;
...
//-----------------------------------------------------------------------------------------------------
//se intenta que este algoritmo devuelva true si esta ordenado por algun criterio
function estaOrdenadoAscendente(L: lista):Boolean;
var 
  tru: boolean;
  auxNum: integer;
...
begin
  ...
  while (valor <> 0) do 
    ...
    if((criterioDeOrden <> 'Nao') {and (criterioDeOrden = 'Ascedente') or (criterioDeOrden = 'Descente')})then
      if(criterioDeOrden = 'Ascedente')then
        Writeln('Ascedente')
